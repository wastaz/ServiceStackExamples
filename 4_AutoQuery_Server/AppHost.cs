    ...
    internal class AppHost : AppHostHttpListenerBase {
        ...
        private void SetupDatabase(IDbConnectionFactory dbFactory) {
            using(var db = dbFactory.OpenDbConnection()) {
                db.CreateTables(true, typeof(Animal), typeof(Owner), typeof(House));

                var pipen = new Animal {AnimalType = AnimalType.Cat, Name = "Pipen"};
                var gravling = new Animal {AnimalType = AnimalType.Cat, Name = "Grävling"};
                var morris = new Animal {AnimalType = AnimalType.Dog, Name = "Morris"};
                db.SaveAll(new[] {pipen, gravling, morris});

                var fredrik = new Owner {Name = "Fredrik", OwnedAnimals = new List<Animal> {pipen, gravling, morris}};
